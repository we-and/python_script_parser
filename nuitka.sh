...
/usr/local/bin/python3 -m nuitka --standalone --macos-create-app-bundle --onefile --follow-imports scripti.py --include-data-dir=examples=examples --include-data-dir=icons=icons --noinclude-data-file=tcl/opt0.4 --noinclude-data-file=tcl/http1.0 --include-module=pandas --include-package=pandas --enable-plugin=tk-inter --macos-disable-console --macos-app-icon=$ICON_FILE --output-dir=$NUITKA_DIST_DIR
