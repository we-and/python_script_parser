text = """BURLY MAN        Come on... hurry up.
BURLY MAN        No!
...
def filter_lines_by_prefix(text, prefix):
    """
    Filters the given text to include only lines that start with the specified prefix.
    
    Args:
        text (str): The complete text to filter.
        prefix (str): The prefix to look for at the start of each line.
        
    Returns:
        str: A string containing only the lines that start with the prefix.
    """
    # Split the text into individual lines
    lines = text.split('\n')
    
    # Keep only lines that start with the specified prefix
...
def save_filtered_text(filtered_text, file_path):
    """
    Saves the filtered text to a file.
    
    Args:
        filtered_text (str): The text to save.
        file_path (str): The path to the file where the text will be saved.
    """
    with open(file_path, 'w', encoding='utf-8') as file:
        file.write(filtered_text)
        print(f"Filtered text has been saved to {file_path}")

filtered=(filter_lines_by_prefix(text,"WADE"))
save_filtered_text(filtered,"filter-WADE.txt")
